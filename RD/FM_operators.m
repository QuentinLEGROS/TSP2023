...
% Fs: signal length - redefined later as L
% Nfft: frequency bins
...
t0  = ((0:2*Lg)'-Lg)/Fs; % evaluation grid of the Gaussian window
gp = (-2*pi/sigma_s^2*t0).*g; % Gaussian window derivative
gpp = (-2*pi/sigma_s^2 + 4*pi^2/sigma_s^4*(t0.^2)).*g; % Gaussian window second derivative
