...
phi1 = 256*t+2700*(t.^2)/2; % freq 1 
phi2 = 768*t+3000*(t.^2)/2; % freq 2 
s1 = exp(2*1i*pi*phi1); % component 1
s2 = exp(2*1i*pi*phi2); % component 2
s_LC = s1 + s2; % 1er MCS - linear chirp
sigma_LC = 0.0188; % noise variance

phi1 = 1400*t+1350/(2*pi)*cos(2*pi*t + pi/2); % freq 1
phi2 = 3400*t+550/(2*pi)*cos(2*pi*t); % freq 2
s1 = exp(2*1i*pi*phi1); % component 1
s2 = exp(2*1i*pi*phi2); % component 2
s_cos = s1 + s2; % 2nd MCS - sinusoidal
sigma_cos = 0.0175; % noise variance

B1 = log(2000);
phi1 = 200*t+2300*(t.^2)/2; % freq 1
phi2 = 2000*t+exp(B1*t)/B1; % freq 2
s1 = exp(2*1i*pi*phi1); % component 1
s2 = exp(2*1i*pi*phi2); % component 2
s_exp = s1 + s2; % 3th MCS - exponential
sigma_exp = 0.0241; % noise variance

%% test
SNR_in = -10;
Nfft = 512;
smooth_p = 1 - 10^(-4); %  weight for further spline approximation
...
s_noise = sigmerge(s_LC, noise, SNR_in); % add noise
