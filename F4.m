...
MCrep =30;          % number of MC realization (20-30 are sufficient to reproduce the figure behaviour)
...
SNRt = snr_range(1):8:snr_range(2);
        ...
        for it = 1:MCrep   %% iterations
            ...
            switch(methods_to_use(ind_met))
                ...
                case 5 %% EM LAP
                    ...
                    [~,~,tf]=Mod_Estim_W_EM_multi(Spect',Fc,Ncomp,1,'Lap',1e-3,step_r,step_v,ifplot,0,0,G);
                    [mask] = compMask(tf,Pnei,N,0);
                    x_hat = zeros(Ncomp,N);
                    for c = 1:Ncomp
                       x_hat(c,:) = real(rectfrgab(tfr .* mask(1:M,:,c), L, M)); 
                 ...
                 case 6 %% EM TV
                    ...
                    step_v = 10;
                    [Fc]=comp_Fc(M,L);Fc = Fc + eps;     %% Data distribution
                    [~,~,tf]=Mod_Estim_W_EM_multi(Spect',Fc,Ncomp,1,'TV',1e-4,step_r,step_v,ifplot,0,0,G);
                    [mask] = compMask(tf,Pnei,N,0);
                    x_hat = zeros(Ncomp,N);
                    for c = 1:Ncomp
                       x_hat(c,:) = real(rectfrgab(tfr .* mask(1:M,:,c), L, M)); 
