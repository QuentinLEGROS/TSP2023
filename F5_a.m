        ...
        for it = 1:MCrep   %% iterations
            ...
            switch(methods_to_use(ind_met))
                ...
                case 5 %% EM LAP
                    ...
                    [~,~,tf]=Mod_Estim_W_EM_multi(Spect',Fc,Ncomp,1,'Lap',1e-1,step_r,step_v,ifplot,0,0,G,1,L);
                    
                case 6 %% EM TV
                    [Fc]=comp_Fc(M,L);Fc = Fc + eps;     %% Data distribution
                    [~,~,tf]=Mod_Estim_W_EM_multi(Spect',Fc,Ncomp,1,'TV',1e-1,step_r,step_v,ifplot,0,0,G,1,L);
                   
                case 7 %% EM LAP + pp
                    [Fc]=comp_Fc(M,L);Fc = Fc + eps;     %% Data distribution
                    [~,~,tf]=Mod_Estim_W_EM_multi(Spect',Fc,Ncomp,1,'Lap',1e-4,step_r,step_v,ifplot,1,0,G,1,L);

                case 8 %% EM TV + pp
                    [Fc]=comp_Fc(M,L);Fc = Fc + eps;     %% Data distribution
                    [~,~,tf]=Mod_Estim_W_EM_multi(Spect',Fc,Ncomp,1,'TV',1e-4,step_r,step_v,ifplot,1,0,G,1,L);
