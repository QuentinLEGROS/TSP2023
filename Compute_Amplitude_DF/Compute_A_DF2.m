function [a]=Compute_A_DF2(x,Fs,m,L,M,n)
...
%   Xtwd=fft(zerophase_signal(twd.*x));
  ...
  Xw=fft(x(n+k) .* w, M) .* exp(D * mm * (n-1 - k_min));
  Xwd=fft(x(n+k) .* wd, M) .* exp(D * mm * (n-1 - k_min));
  Xtw=fft(x(n+k) .* tw, M) .* exp(D * mm * (n-1 - k_min));
  Xwdd=fft(x(n+k) .* wdd, M) .* exp(D * mm * (n-1 - k_min));
  Xtwd=fft(x(n+k) .* twd, M) .* exp(D * mm * (n-1 - k_min));
